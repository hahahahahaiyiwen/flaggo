...
It does own the shared parsing of the Development-only resource identity used
by both hosts. `Flaggo:Authentication:LocalDevelopmentAppId` and
`Flaggo:Authentication:LocalDevelopmentEnvironment` default to the existing
`tetris-demo`/`dev` scope and must remain non-empty when overridden.
